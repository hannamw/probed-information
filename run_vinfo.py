...
from probing import Probe, get_reps
...
train, valid, test = get_reps(args.model_name, batch_size=args.batch_size)
    ...
    for probe_run in range(args.probe_runs):
        for word in range(WORDS_IN_SENTENCE):
            ...
            for layer in range(n_layers):
                ...
                path = Path(f'probes/{args.model_name}/run-{probe_run}/word-{word}/layer-{layer}')
